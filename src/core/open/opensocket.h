...
public:
	...
	static int UDPAddressToIpPort(const char* address, std::string& addr, int& port);
	static int IpPortToUDPAddress(const char* addr, int port, uint8_t* udp_address, int upd_size);

	void socketInfo(std::vector<Info>& vectInfo);
	inline bool isRunning() { return isRunning_; }

	static void Sleep(int64_t milliSecond);
	static const std::string DomainToIp(const std::string& domain);
